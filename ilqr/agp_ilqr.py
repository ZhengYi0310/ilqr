...
class AGP_iLQR(BaseTrajOptimizer):
    """Finite Horizon Iterative Linear Quadratic Regulator with GP dynamics model"""

    def __init__(self, sys_dynamics_GP, cost_func, time_steps, max_reg=1e10, use_second_order=False, control_limit=False, **kwargs):
        """
        ...
        if control_limit is True:
            self.lower_bound = kwargs['lower']
            self.upper_bound = kwargs['upper']
        ...
        super(AGP_iLQR, self).__init__()

    def _update_dynamics(self, xs, us, Y):
        X = np.hstack([xs, us])
        self.training_set = np.append(np.hstack([xs, us]), axis=0)
        ### perform k-means to get the new U
        if (self.training_set.shape[0] >= self.pseudo_num):
            kmeans = KMeans(n_clusters=self.pseudo_num).fit(self.training_set)
            new_U = kmeans._cluster_centers
        else:
            new_U = self.training_set
        assert (len(self.dynamics) == Y.shape[0]), "GP model should be assigned to each dimension of the output"

        start_time = time.time()
        for i in range(0, len(self.dynamics)):
            self.dynamics[i].add_data(self, X, Y[:, i])
            self.dynamics[i] = pygp.inference.DTC.from_gp(self.dynamics[i], new_U)
            ##optimize the gp
            pygp.optimize(self.dynamics[i])
        print("Updating DTC GP dynamics model...., takes about {:.5}s".format(str(time.time() - start_time)))

    def _dynamics_rollout(self, x0, us):
        '''
        "Compute a trajectory rollout based on the DTC gaussian process model"
        ...
        x_last_ = np.hstack([xs, us[0, :]])
        for i in range(0, self.N):
            d_xs_next = np.array([self.dynamics[i].posterior(x_last_) for i in range (0, len(self.dynamics))])
            xs = np.append(xs, xs + d_xs_next, axis=0)
            x_last_ = np.hstack(np.hstack([xs[i, :], us[i, :]]))
    ...
    def _forward_pass(self, xs, us, k, K, alpha=1):
        ...
        for i in range(0, self.N):
            ...
            x_last_ = np.hstack([xs_new[i], us[i, :]])
            d_xs_next = np.array([self.dynamics[i].posterior(x_last_) for i in range(0, len(self.dynamics))])
            xs_new[i + 1] = xs_new[i] + d_xs_next[i]
    ...
    def _compute_Q_terms(self, x, u, V_x, V_xx, i):
        ...
        :return: 
        """
        delta_f = self.dynamics[0].posterior(np.hstack([x, u]), grad=True)
        for i in range(1, len(self.dynamics)):
            delta_f = np.append(self.dynamics[i].posterior(np.hstack([x, u]), grad=True)[2], axis=0)

        f_x = np.eye(len(self.dynamics)) + delta_f[0:, 0:len(self.dynamics)]
        f_u = delta_f[0:, len(self.dynamics):]
        ...
        Q_uu = l_uu + np.dot(np.dot(f_u.T, reg), f_u) #+ self.mu_ * np.eye(u.shape[0])
    ...
    def update_u(self, x0, u_seq_init, n_interations=200, tolerance=1e-6, on_iteration=None, line_search_steps=10, scale=0.8):
        """
