...
class AutogradCost(Cost):
    ...
    def __init__(self, l, l_terminal, x, u, i=None, **kwargs):
        ...
        self._l = l
        self._l_x = jacobian(self._l, 0)#self._J[0]
        self._l_u = jacobian(self._l, 1)#self._J[1]
        self._l_xx = jacobian(self._l_x, 0)#self._Q[0]
        self._l_ux = jacobian(self._l_u, 0)#self._Q[1]
        self._l_uu = jacobian(self._l_u, 1)
        ...
        self._l_x_terminal = jacobian(self._l_terminal, 0)#self._J_terminal[0] #[jacobian_x]
        self._l_xx_terminal = jacobian(self._l_x_terminal, 0)#self._Q_terminal[0] #[hessian_xx]
...
#         :return:
#         """
#         Q = self.Q_terminal_ if terminal else self.Q_
#         R = self.R_
#         x_diff = x - self.x_goal_
...
#         """
#         return np.zeros((self.R_.shape[0], self.Q_.shape[0]))
#
#     def l_uu(self, x, u, i, terminal=False):
...
# u = np.array([2., 0.5, -3])
# # print np.vstack((x, u))
# fun_vec = lambda x, u: [np.dot(x, x.T) + u[1],
#                         np.dot(u, u.T) * 2 + x[4] * x[2] + x[3] * x[3] - u[1] * u[1] + u[0] * u[2] + u[0] * x[1]]
# # for f in fun_vec:
# #    print f(x, u)
# # print jacobian(fun_vec, 0)
# fun = lambda x, u: np.dot(x, x.T) + np.dot(u, u.T) * 2 + x[4] * x[2] + x[3] * x[3] - u[1] * u[1] + u[0] * u[2] + u[0] * \
#                                                                                                                  x[1]
